...
pub enum HookReadError {
    ...
    InputTooLarge {
        /// Number of bytes drained into the bounded metadata buffer. This is
        /// capped at [`MAX_OVERSIZED_METADATA_BYTES`], so for a larger payload
        /// it understates the true size.
        len: usize,
...
/// Bounded extension used only to recover a late Codex++ capability marker.
const MAX_OVERSIZED_METADATA_BYTES: usize = 8 * 1024 * 1024;
...
pub fn read_hook_input(max_bytes: usize) -> Result<HookInput, HookReadError> {
    ...
    if buf.len() > max_bytes {
        // Keep draining into a bounded metadata buffer. Only its first 4 MiB
        // reaches command evaluation; the remainder can recover a late
        // Codex++ capability marker without making input consumption unbounded.
        // Best-effort: a read error here just shortens the scan buffer, it
        // never changes the (already decided) oversized verdict.
        if buf.len() < MAX_OVERSIZED_METADATA_BYTES {
            let remaining = (MAX_OVERSIZED_METADATA_BYTES - buf.len()) as u64;
            let stdin = io::stdin();
            let mut handle = stdin.lock().take(remaining);
            let _ = handle.read_to_end(&mut buf);
        }
        let len = buf.len();
        let metadata_prefix = String::from_utf8_lossy(&buf);
        let permission_decision_ask_supported =
            permission_decision_ask_supported_from_truncated_json(&metadata_prefix);
        let scan_len = buf.len().min(MAX_OVERSIZED_SCAN_BYTES);
        ...
        return Err(HookReadError::InputTooLarge {
            len,
            prefix: String::from_utf8_lossy(&buf[..scan_len]).into_owned(),
...
/// Recover Codex++'s top-level `ask` capability from a truncated hook envelope.
#[must_use]
pub fn permission_decision_ask_supported_from_truncated_json(prefix: &str) -> Option<bool> {
    const KEY: &[u8] = b"\"permission_decision_ask_supported\"";
    ...
    while index < bytes.len() {
        ...
        match byte {
            ...
            b'"' if depth == 1 && bytes[index..].starts_with(KEY) => {
                let mut value_start = index + KEY.len();
                ...
                if bytes.get(value_start) == Some(&b':') {
                    ...
                    let value = if bytes[value_start..].starts_with(b"true") {
                        Some((true, 4))
                    ...
                    if let Some((value, width)) = value {
                        let after_value = value_start + width;
                        if bytes.get(after_value).is_none_or(|next| {
...
pub fn write_indeterminate_to(
    ...
    match protocol {
        protocol @ (HookProtocol::ClaudeCompatible | HookProtocol::CodexAsk) => {
            let (permission_decision, permission_decision_reason) =
                if protocol == HookProtocol::CodexAsk {
                    ("ask", Cow::Borrowed(reason))
                } else {
...
mod tests {
    ...
    fn test_write_indeterminate_denies_when_unverified_decision_is_deny() {
        ...
        let cases = [
            ...
            (HookProtocol::CodexAsk, "ask", false),
