...
fn generate_config_with_packs(packs: &[String]) -> String {
    ...
    let mut lines = vec![
        ...
        "# https://github.com/Pimpmuckl/destructive_command_guard".to_string(),
...
fn self_update_unix(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let script_url = format!(
        "https://raw.githubusercontent.com/Pimpmuckl/destructive_command_guard/{normalized_tag}/install.sh"
    );
    ...
    let sha_url = format!(
        "https://github.com/Pimpmuckl/destructive_command_guard/releases/download/{normalized_tag}/install.sh.sha256"
    );
...
fn self_update_windows(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let normalized_tag = update_installer_tag(requested_version.as_deref())?;
    let script_url = format!(
        "https://raw.githubusercontent.com/Pimpmuckl/destructive_command_guard/{normalized_tag}/install.ps1"
    );
    let sha_url = format!(
        "https://github.com/Pimpmuckl/destructive_command_guard/releases/download/{normalized_tag}/install.ps1.sha256"
    );
